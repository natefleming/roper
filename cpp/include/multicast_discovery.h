...
public:
	...
	MulticastDiscovery(const std::string& listen_address,
			const std::string& multicast_address, short mulitcast_port);

	MulticastDiscovery(const std::string& multicast_address,
	...
	public:
		...
		virtual std::shared_ptr<MulticastDiscovery> build() override {
			return std::make_shared<MulticastDiscovery>(bind_address_,
					multicast_address_, port_);
		}

		Builder& with_bind_endpoint(const std::string& bind_address,
				short port) {
